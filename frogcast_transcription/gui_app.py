...
class AudioTranscriptionApp:
    def __init__(self, root):
        ...
        self.logo_label.grid(row=0, column=0, rowspan=3, padx=10, pady=10)
        ...
        self.audio_path_label.grid(row=0, column=3, columnspan=2, padx=10, pady=10)
        ...
        self.output_folder_button.grid(row=2, column=2, padx=10, pady=10)

        self.transcribe_button = tk.Button(root, text="Transcribe", command=self.transcribe_audio)
        self.transcribe_button.grid(row=3, column=1, columnspan=2, padx=10, pady=10)
    ...
    def transcribe_audio(self):
        if hasattr(self, "audio_path") and hasattr(self, "output_folder"):
            output_filename = "transcribed_text_with_timestamps.txt"

            subprocess.run(["python", "transcribe_audio_whisper.py", self.audio_path, self.output_folder, output_filename])
